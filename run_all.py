...
def create_notebooks() -> list[Path]:
    """Return the checked-in self-contained notebooks without rebuilding their cells.

    The notebooks are intentionally authored as reviewer-facing workflows: tables,
    figures, and model artifacts are created in the sections where they are first
    used. Reconstructing them from a central script would reintroduce the old
    all-at-once artifact-generation pattern that the submitted notebooks avoid.
    """
    NOTEBOOK_DIR.mkdir(parents=True, exist_ok=True)
    notebook_paths = [
        NOTEBOOK_DIR / "01_milestone1_eda.ipynb",
        NOTEBOOK_DIR / "02_milestone2_modeling.ipynb",
    ]
    missing = [path for path in notebook_paths if not path.exists()]
    if missing:
        missing_list = ", ".join(rel_path(path) for path in missing)
        raise FileNotFoundError(
            "Expected self-contained notebook artifact(s) are missing: "
            f"{missing_list}. Restore the notebooks from source control before running the full pipeline."
        )
    ...
    for nb_path in notebook_paths:
        ...
        notebook.metadata.setdefault("language_info", {"name": "python", "pygments_lexer": "ipython3"})
